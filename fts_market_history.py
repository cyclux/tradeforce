...
class MarketHistory:
    ...
    def get_market_history(
        ...
        self.df_market_history.sort_index(inplace=True)
        if fill_na and not self.fts_instance.backend.is_filled_na:
            df_fill_na(self.df_market_history)
            self.fts_instance.backend.is_filled_na = True
    ...
    async def update(self, history_data=None, start=None, end=None):
        ...
        if history_data is not None:
            ...
            for interval in timestamp_intervals:
                ...
                i_start = interval[0]
                i_end = interval[1]
                df_history_update_interval = await self.fts_instance.market_updater_api.update_market_history(
    ...
    def get_local_candle_timestamp(self, position="latest", skip=0):
        ...
        elif self.config.backend == "mongodb" and not self.fts_instance.backend.is_collection_new:
