"""
Default script which runs on `docker compose up`.

Either replace / edit this file to your needs.

Or edit the `docker-compose.yml` file (entrypoint) to run a different script.
In this case, note the comments in the `docker_tradeforce.volumes` section:
You may need to adjust the volume mapping.

Note:
    This script / configuration is a copy of `examples/simulator_simple_dict.py` and can
    be edited to any other script or configuration.
