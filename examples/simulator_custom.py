...
This example details the implementation of the default trading strategies.

First pre_process() gets called once per dataset_type (train/val) before iterating
over the respective dataset: sell_strategy() and buy_strategy() are called on
each iteration step / record of the dataset.

The default strategy functions can be used as a template for custom strategies or
completely replaced by more sophisticated strategy implementations.

NOTE: buy_strategy() and sell_strategy() are accelerated using Numba. Numba
transpiles Python code into machine code and then JIT compiles it, which results
in a significant performance boost especially for functions like buy_strategy()
and sell_strategy(), which are called on each iteration step of the trading sim.

However, this performance boost comes at the cost of convenience: Numba is not
compatible with all Python functionalities and requires some adjustments and
considerations in the code. For example, Numba does not support Pandas DataFrames
or any other external libary except NumPy. Further, it is important to be aware of
supported types and their limitations.
...
# For reference see buy_strategy()
# ---------------------------------------------------
